...
cMap::cMap() {
	...
	iScrollSpeed=10;
	cellCalculator = NULL;
	randomMapGenerator = new cRandomMapGenerator();
}

cMap::~cMap() {
	if (cellCalculator) delete cellCalculator;
	if (randomMapGenerator)	delete randomMapGenerator;
}
...
{
	randomMapGenerator->smoothAroundCell(c);
}

// smooths out ROCK/SAND/SPICE/HILL/SPICEHILL stuff
void cMap::smooth() {
	randomMapGenerator->smoothMap();
}

void cMap::soft()
{
	randomMapGenerator->removeSingleRockSpots();
}

//
void cMap::smooth_cell(int c) {
	randomMapGenerator->smoothCell(c);
}
