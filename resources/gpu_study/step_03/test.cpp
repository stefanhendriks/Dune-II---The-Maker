...
// Structure de Vertex (8 octets : position 2D)
struct Vertex {
...
SDL_GPUShader* LoadShader(SDL_GPUDevice* device, const std::string& filename, SDL_GPUShaderStage stage) {
    ...
    // Le fragment shader consomme un storage buffer pour la couleur finale.
    shaderInfo.num_samplers = 0;
    shaderInfo.num_uniform_buffers = 0;
    shaderInfo.num_storage_buffers = (stage == SDL_GPU_SHADERSTAGE_FRAGMENT) ? 1 : 0;
...
int main(int argc, char* argv[]) {
    ...
    bufferDesc.slot = 0;
    bufferDesc.pitch = sizeof(Vertex); // 8 octets (2x float pos)
    ...
    FragmentColor fragmentColor = {1.0f, 1.0f, 1.0f, 1.0f};
    ...
    SDL_GPUTransferBuffer* vertexTransferBuffer = SDL_CreateGPUTransferBuffer(device, &transferBufferInfo);
    ...
    SDL_GPUBufferCreateInfo fragmentColorBufferInfo{};
    fragmentColorBufferInfo.usage = SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ;
    fragmentColorBufferInfo.size = sizeof(fragmentColor);
    fragmentColorBufferInfo.props = 0;
    SDL_GPUBuffer* fragmentColorBuffer = SDL_CreateGPUBuffer(device, &fragmentColorBufferInfo);
    if (!fragmentColorBuffer) {
        std::cerr << "Erreur création fragment storage buffer : " << SDL_GetError() << "\n";
    ...
    SDL_GPUTransferBufferCreateInfo fragmentTransferInfo{};
    fragmentTransferInfo.usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD;
    fragmentTransferInfo.size = sizeof(fragmentColor);
    fragmentTransferInfo.props = 0;
    SDL_GPUTransferBuffer* fragmentTransferBuffer = SDL_CreateGPUTransferBuffer(device, &fragmentTransferInfo);
    if (!fragmentTransferBuffer) {
        std::cerr << "Erreur création fragment transfer buffer : " << SDL_GetError() << "\n";
        SDL_ReleaseGPUBuffer(device, fragmentColorBuffer);
        SDL_ReleaseGPUTransferBuffer(device, vertexTransferBuffer);
        SDL_ReleaseGPUBuffer(device, vertexBuffer);
        SDL_ReleaseGPUGraphicsPipeline(device, pipeline);
        SDL_DestroyGPUDevice(device);
    ...
    // Upload statique des sommets: inutile de recopier les vertices à chaque frame.
    ...
    if (!vertexMapped) {
        ...
        SDL_ReleaseGPUTransferBuffer(device, fragmentTransferBuffer);
        SDL_ReleaseGPUBuffer(device, fragmentColorBuffer);
        SDL_ReleaseGPUTransferBuffer(device, vertexTransferBuffer);
        SDL_ReleaseGPUBuffer(device, vertexBuffer);
        SDL_ReleaseGPUGraphicsPipeline(device, pipeline);
    ...
    SDL_memcpy(vertexMapped, triangleVertices, sizeof(triangleVertices));
    ...
    SDL_GPUTransferBufferLocation initSourceLocation{};
    initSourceLocation.transfer_buffer = vertexTransferBuffer;
    initSourceLocation.offset = 0;

    SDL_GPUBufferRegion initDestinationRegion{};
    initDestinationRegion.buffer = vertexBuffer;
    initDestinationRegion.offset = 0;
    initDestinationRegion.size = sizeof(triangleVertices);

    SDL_UploadToGPUBuffer(initCopyPass, &initSourceLocation, &initDestinationRegion, false);
    ...
    while (running) {
        ...
        fragmentColor.r = 0.3f + 0.7f * pulse;
        fragmentColor.g = 0.3f + 0.7f * ((std::sin(time * 2.4f + 2.0f) + 1.0f) * 0.5f);
        fragmentColor.b = 0.3f + 0.7f * ((std::sin(time * 3.6f + 4.0f) + 1.0f) * 0.5f);
        fragmentColor.a = 1.0f;

        SDL_GPUCommandBuffer* cmdBuffer = SDL_AcquireGPUCommandBuffer(device);
        if (!cmdBuffer) continue;

        void* fragmentMapped = SDL_MapGPUTransferBuffer(device, fragmentTransferBuffer, true);
        if (!fragmentMapped) {
            std::cerr << "Erreur map fragment transfer buffer : " << SDL_GetError() << "\n";
            SDL_SubmitGPUCommandBuffer(cmdBuffer);
            continue;
        }
        SDL_memcpy(fragmentMapped, &fragmentColor, sizeof(fragmentColor));
        SDL_UnmapGPUTransferBuffer(device, fragmentTransferBuffer);

        SDL_GPUCopyPass* copyPass = SDL_BeginGPUCopyPass(cmdBuffer);
        SDL_GPUTransferBufferLocation fragmentSourceLocation{};
        fragmentSourceLocation.transfer_buffer = fragmentTransferBuffer;
        fragmentSourceLocation.offset = 0;

        SDL_GPUBufferRegion fragmentDestinationRegion{};
        fragmentDestinationRegion.buffer = fragmentColorBuffer;
        fragmentDestinationRegion.offset = 0;
        fragmentDestinationRegion.size = sizeof(fragmentColor);

        SDL_UploadToGPUBuffer(copyPass, &fragmentSourceLocation, &fragmentDestinationRegion, false);
        ...
        if (swapchainTexture != nullptr) {
            ...
            SDL_GPUBuffer* fragmentStorageBuffers[1] = { fragmentColorBuffer };
