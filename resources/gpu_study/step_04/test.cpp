...
int main(int argc, char* argv[]) {
    ...
    const float right = -1.0f + (2.0f * static_cast<float>(imageWidth) / static_cast<float>(windowWidth));
    const float bottom = -1.0f;
    const float top = -1.0f + (2.0f * static_cast<float>(imageHeight) / static_cast<float>(windowHeight));
