...
SDL_GPUShader* LoadShader(SDL_GPUDevice* device, const std::string& filename, SDL_GPUShaderStage stage) {
    ...
    // Le fragment shader utilise un cbuffer (slot b0) pour recevoir un float animé.
    shaderInfo.num_samplers = 0;
    shaderInfo.num_uniform_buffers = (stage == SDL_GPU_SHADERSTAGE_FRAGMENT) ? 1 : 0;
...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        if (swapchainTexture != nullptr) {
            ...
            // Oscille dans le temps pour démontrer le passage d'un float CPU -> fragment shader.
            const float timeSeconds = static_cast<float>(SDL_GetTicks()) / 1000.0f;
            fragmentUniforms.redWave = timeSeconds * 2.0f;

            SDL_PushGPUFragmentUniformData(cmdBuffer, 0, &fragmentUniforms, sizeof(fragmentUniforms));
