...
int main() {
	...
	for (i = 0; i < NUM_PARTICLES; i++) {
		...
		clear_to_color(particle[i]->bmp, bitmap_mask_color(particle[i]->bmp));
	...
	do {
		...
		if (use_alleg) {		
			for (i = 0; i < NUM_PARTICLES; i++) {
				set_trans_blender(0, 0, 0, particle[i]->a);
				draw_trans_sprite(buffer, particle[i]->bmp, particle[i]->x - particle[i]->bmp->w/2, particle[i]->y - particle[i]->bmp->h/2);
			}
		}
		else {
			for (i = 0; i < NUM_PARTICLES; i++)
				fblend_trans(particle[i]->bmp, buffer, particle[i]->x - particle[i]->bmp->w/2, particle[i]->y - particle[i]->bmp->h/2, particle[i]->a);
		}
