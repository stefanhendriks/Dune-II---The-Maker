            ...
            if (unit[i].iType == SANDWORM && unit[i].iPlayer == ID)
            {
                ...
                if (unit[i].iAction == ACTION_GUARD)
                {
                    ...
                    for (int iTries=0; iTries < 10; iTries++)
                    {
                        ...
                        if (map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_SAND ||
                            map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_HILL ||
                            map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_SPICE ||
                            map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL)
                        {
    ...
    for (int i=0; i < MAX_UNITS; i++)
        if (unit[i].isValid())
            if (unit[i].iPlayer == iAttackPlayer)
			{
				...
				{
				char msg[255];
				sprintf(msg, "AI %d (House %d) -> Visible = %d", iPlayer, player[iPlayer].getHouse(), mapUtils->isCellVisibleForPlayerId(iPlayer, unit[i].iCell));
				logbook(msg);
				}
