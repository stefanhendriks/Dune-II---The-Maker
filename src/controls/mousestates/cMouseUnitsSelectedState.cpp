...
void cMouseUnitsSelectedState::evaluateSelectedUnits()
{
    ...
    m_selectedUnits.erase(
        std::remove_if(
            ...
            m_selectedUnits.end(),
    [this](int id) {
        return !g_Unit[id].isValid() || // no (longer) valid
               !g_Unit[id].belongsTo(m_player) || // no longer belongs to player
               g_Unit[id].isHidden(); // hidden (entered structure, etc). Forget it then.
    }),
    m_selectedUnits.end()
    );
}
