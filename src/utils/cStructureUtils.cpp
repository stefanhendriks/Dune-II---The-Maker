...
int cStructureUtils::getHeightOfStructureTypeInCells(int structureType)
{
    ...
    return structureInfos[structureType].bmp_height / TILESIZE_HEIGHT_PIXELS;
}
...
int cStructureUtils::getWidthOfStructureTypeInCells(int structureType)
{
    assert(structureType >= 0);
    assert(structureType < MAX_STRUCTURETYPES);
    return structureInfos[structureType].bmp_width / TILESIZE_WIDTH_PIXELS;
}
...
int cStructureUtils::findStructureToDeployUnit(cPlayer *pPlayer, int structureType)
{
    ...
    if (game.isDebugMode()) {
        logbook(std::format(
                    "Looking for primary building (type {}, name {}, pPlayer {})", structureType, structureInfos[structureType].name, playerId));
    }
...
void cStructureUtils::putStructureOnDimension(int dimensionId, cAbstractStructure *theStructure)
{
    ...
    for (int w = 0; w < theStructure->getWidth(); w++) {
        for (int h = 0; h < theStructure->getHeight(); h++)	{
            int xOfStructureCell = game.getMap().getCellX(cellOfStructure);
            int yOfStructureCell = game.getMap().getCellY(cellOfStructure);

            int iCell = game.getMap().getGeometry().makeCell(xOfStructureCell + w, yOfStructureCell + h);

            game.getMap().cellSetIdForLayer(iCell, dimensionId, theStructure->getStructureId());
        }
    }
}
