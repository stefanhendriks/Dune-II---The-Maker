...
unsigned char cSeedMapGenerator::spicemap[256] = {
...
unsigned char cSeedMapGenerator::spicemap2[256] = {
    0x7f,0x7e,0x7e,0x7e,0x7e,0x7e,0x7d,0x7d,0x7c,0x7b,0x7b,0x7a,0x79,0x78,0x77,0x76,
    0x75,0x74,0x72,0x71,0x70,0x6e,0x6c,0x6b,0x69,0x67,0x66,0x64,0x62,0x60,0x5e,0x5b,
    0x59,0x57,0x55,0x52,0x50,0x4e,0x4b,0x49,0x46,0x43,0x41,0x3e,0x3b,0x39,0x36,0x33,
    0x30,0x2d,0x2a,0x27,0x24,0x21,0x1e,0x1b,0x18,0x15,0x12,0xf,0xc,0x9,0x6,0x3,
...
void cSeedMapGenerator::spreadMatrix(cell map[64][64])
{
    ...
    for (y = 0; y < 64; y += 4) {
        for (x = 0; x < 64; x += 4) {
            ...
            if (diag) offs = offsets2;
            else offs = offsets2+(SMG_PAIRCOUNT*4);
            diag = 1 - diag;
            for (i = 0; i < 21; i++) {
                bx = x + *(offs);
                by = y + *(offs+1);
                ...
                medy = (ey + by)/2;
                bx &= 0x3f;
                ex &= 0x3f; /* so x position never exceeds 0x3F */
    ...
    for (int y = 0; y < cSeedMap::kMapHeight; y++) {
        for (int x = 0; x < cSeedMap::kMapWidth; x++) {
            ...
            switch (type) {
                //fixed offset by JPEXS
                case SMG_SAND:
                    d2tmType = TERRAIN_SAND;
                    break;
                case SMG_ROCK:
                    d2tmType = TERRAIN_ROCK;
                    break;
                case SMG_DUNES:
                    d2tmType = TERRAIN_HILL;
                    break;
                case SMG_MOUNTAINS:
                    d2tmType = TERRAIN_MOUNTAIN;
                    break;
                case SMG_SPICE:
                    d2tmType = TERRAIN_SPICE;
                    break;
                case SMG_MUCHSPICE:
                    d2tmType = TERRAIN_SPICEHILL;
                    break;
            }
