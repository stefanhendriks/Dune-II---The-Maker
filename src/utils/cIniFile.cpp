...
bool cIniFile::load(const std::string &config)
{
    cLogger *logger = cLogger::getInstance();
    m_fileName = config;
    logger->log(LOG_INFO, COMP_GAMEINI, "(cIniFile)", std::format("Load file {}", m_fileName));
    ...
    while (!in.eof()) {
        ...
        logger->log(LOG_WARN, COMP_GAMEINI, "(cIniFile)", std::format("Error {} or no section found", line));
    }
    return true;
}
