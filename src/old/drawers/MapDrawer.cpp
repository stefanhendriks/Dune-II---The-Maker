	...
	for (int iStartX = camera->getX(); iStartX < camera->getEndX(); iStartX++) {
		...
		for (int iStartY = camera->getY(); iStartY < camera->getEndY(); iStartY++) {
			...
			if (map->cell[iCell].smudgeTerrainTypeGfxDataIndex > -1 && map->cell[iCell].smudgeTileToDraw > -1) {
				masked_blit((BITMAP *) gfxdata[SMUDGE].dat, bmp_screen, map->cell[iCell].smudgeTileToDraw * 32,
						map->cell[iCell].smudgeTerrainTypeGfxDataIndex * 32, iDrawX, iDrawY, 32, 32);
