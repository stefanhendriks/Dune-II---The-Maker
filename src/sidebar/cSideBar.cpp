...
{
    game.global_drawManager->setMessage(item->getInfo());
}
...
void cSideBar::onMouseAt(const s_MouseEvent &event)
{
    ...
    game.global_drawManager->setKeepMessage(isMouseOverSidebarValue);
    ...
    cBuildingListDrawer *buildingListDrawer = game.global_drawManager->getBuildingListDrawer();
