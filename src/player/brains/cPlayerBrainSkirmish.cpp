...
void cPlayerBrainSkirmish::onMyStructureAttacked(const s_GameEvent &event)
{
    ...
    if (unitIdThatAttacks > -1) {
        ...
        cUnit originUnit = game.getUnits()[unitIdThatAttacks];
...
{
    auto position = std::find_if(m_missions.begin(), m_missions.end(), [&id](const cPlayerBrainMission & mission) {
        return mission.getUniqueId() == id;
    });
    ...
    if (!placeResult.unitIds.empty()) {
        for (auto &unitId : placeResult.unitIds) {
            ...
            if (aUnit.getPlayer() == player) {
                ...
                if (aUnit.isIdle()) {
                    aUnit.move_to(global_map.getRandomCellFrom(aUnit.getCell(), 3));
                }
            }
            else {
...
void cPlayerBrainSkirmish::onMyUnitAttacked(const s_GameEvent &event)
{
    ...
    if (event.originType == eBuildType::UNIT) {
        ...
        threat = &game.getUnits()[event.originId];
    }

    cUnit &victim = game.getUnits()[event.entityID];
