...
void cSelectYourNextConquestState::thinkFast()
{
    ...
    if (state == eRegionState::REGSTATE_INTRODUCTION) {
        ...
        if (regionSceneState == SCENE_INIT) {
            ...
            game.m_drawManager->setMessage("3 Houses have come to Dune.");
            ...
            if (!hasMessage && iRegionSceneAlpha >= 255) {
                game.m_drawManager->setMessage("To take control of the land.");
            ...
            if (!hasMessage && iRegionSceneAlpha >= 255) {
                game.m_drawManager->setMessage("That has become divided.");
    ...
    if (state == eRegionState::REGSTATE_CONQUER_REGIONS || state == eRegionState::REGSTATE_INTRODUCTION) {
        ...
        for (int i = 0; i < MAX_REGIONS; i++) {
            ...
            if (region.iHouse != houseThatConquersTheRegion) {
                ...
                if ((isRegionTextGiven && !game.m_drawManager->hasMessage()) || isRegionTextEmpty) {
                    // set this up
                    ...
                    if (isRegionTextGiven) {
                        game.m_drawManager->setMessage(regionTextString);
                    }
...
void cSelectYourNextConquestState::draw() const
{
    ...
    game.m_drawManager->drawMessageBar();

    m_guiBtnToMissionSelect->draw();

    game.m_drawManager->drawMouse();
}
...
void cSelectYourNextConquestState::drawStateConquerRegions() const   // draw dune first
{
    ...
    if (isFinishedConqueringRegions && !game.m_drawManager->hasMessage()) {
        game.m_drawManager->setMessage("Select your next region.", true);
    }
}
...
{
    game.m_drawManager->regionInit(offsetX, offsetY);
...
int cSelectYourNextConquestState::regionOver(int mouseX, int mouseY)
{
    ...
    game.m_drawManager->setMessage(std::format("region {}",c-1));
    return c - 1;
}
