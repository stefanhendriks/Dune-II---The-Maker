...
void cCreditsState::prepareCrawlerLines()
{
    ...
    m_lines.push_back(s_CreditLine {
        .name = "",
        .txt = std::format("Version {}", D2TM_VERSION),
        .color = Color::yellow(),
...
{
    global_renderDrawer->renderSprite(m_duneBmp, m_duneCoordinates.x, m_duneCoordinates.y);

    int halfScreen = m_game.m_screenW / 2;

    // draw crawler
    global_renderDrawer->renderSprite(m_titleBmp, m_titleX, m_crawlerY);
    int textCrawlY = m_crawlerY + m_titleHeight;
    for (auto &line : m_lines) {
        if (line.name.empty()) {
