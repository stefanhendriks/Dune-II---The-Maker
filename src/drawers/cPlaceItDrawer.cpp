    ...
    if (!bWithinBuildDistance) {
        ...
        for (int iX=0; iX < cellWidth; iX++) {
            for (int iY=0; iY < cellHeight; iY++) {
                ...
                float desiredWidth = game.global_mapCamera->getZoomedTileWidth();
                float desiredHeight = game.global_mapCamera->getZoomedTileHeight();
...
void cPlaceItDrawer::drawStructureIdAtMousePos(cBuildingListItem *itemToPlace)
{
    ...
    int scaledWidth = game.global_mapCamera->factorZoomLevel(width);
    int scaledHeight = game.global_mapCamera->factorZoomLevel(height);
