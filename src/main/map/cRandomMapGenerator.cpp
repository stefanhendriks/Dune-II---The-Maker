...
void cRandomMapGenerator::generateRandomMap() {
	...
	if (game.iSkirmishStartPoints == 2) {
		iDistance = 32;
	} else if (game.iSkirmishStartPoints == 3) {
		iDistance = 26;
	} else if (game.iSkirmishStartPoints == 4) {
		iDistance = 22;
	}
	...
	for (int x = 0; x < 64; x++)
		for (int y = 0; y < 64; y++) {
			...
			if (map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL)
				iColor = makecol(180, 90, 25); // bit darker
			if (map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SPICE)
