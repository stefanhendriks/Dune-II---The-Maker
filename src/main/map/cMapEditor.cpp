	...
	for (int i = 0; i < size; i++) {
		...
		if (c > -1) {
			...
			if (terrainType == TERRAIN_SPICE) {
				if ((terrainTypeOfNewCell != TERRAIN_ROCK) && (terrainTypeOfNewCell != TERRAIN_SLAB) && (terrainTypeOfNewCell != TERRAIN_MOUNTAIN)
...
int cMapEditor::getDefaultTerrainIndex(bool up, bool down, bool left, bool right) {
	...
	int value = -1;
	if (l == false && a == false && r == false && u == false)
		value = 10; // lonely tile
	if (l == true && a == false && r == false && u == false)
		value = 12; // at the left only
	if (l == false && a == false && r == true && u == false)
		value = 13; // at the right only
	if (l == false && a == true && r == false && u == false)
		value = 14; // at the top only
	if (l == false && a == false && r == false && u == true)
		value = 15; // at the bottom only
	if (l == true && a == false && r == true && u == false)
		value = 11; // one way connection  (left/right)
	if (l == false && a == true && r == false && u == true)
		value = 16; // one way connection  (up/down)
	...
	if (l == true && a == true && r == false && u == true)
		value = 3; // border LEFT
	if (l == false && a == true && r == true && u == true)
		value = 2; // border RIGHT
	if (l == true && a == true && r == true && u == false)
		value = 5; // border UNDER
	if (l == true && a == false && r == true && u == true)
		value = 4; // border ABOVE

	// accompanied rock (all sides rock too)
	if (l == true && a == true && r == true && u == true)
		value = 1; // not so lonely rock
...
int cMapEditor::smoothRockCell(int cell) {
	...
	// rock above?
	a = isAboveSpecificTerrainType(cell, TERRAIN_ROCK) || isAboveSpecificTerrainType(cell, TERRAIN_SLAB) || isAboveSpecificTerrainType(cell, TERRAIN_MOUNTAIN)
			|| isAboveSpecificTerrainType(cell, TERRAIN_WALL);

	d = isBelowSpecificTerrainType(cell, TERRAIN_ROCK) || isBelowSpecificTerrainType(cell, TERRAIN_SLAB) || isBelowSpecificTerrainType(cell, TERRAIN_MOUNTAIN)
			|| isBelowSpecificTerrainType(cell, TERRAIN_WALL);

	l = isLeftSpecificTerrainType(cell, TERRAIN_ROCK) || isLeftSpecificTerrainType(cell, TERRAIN_SLAB) || isLeftSpecificTerrainType(cell, TERRAIN_MOUNTAIN)
			|| isLeftSpecificTerrainType(cell, TERRAIN_WALL);

	r = isRightSpecificTerrainType(cell, TERRAIN_ROCK) || isRightSpecificTerrainType(cell, TERRAIN_SLAB) || isRightSpecificTerrainType(cell, TERRAIN_MOUNTAIN)
	...
	for (int x = startX; x < endX; x++) {
		for (int y = startY; y < endY; y++) {
			...
			if (terrainType == TERRAIN_ROCK) {
				...
				if (isLeftSpecificTerrainType(cll, TERRAIN_ROCK))
					iC++;
				if (isRightSpecificTerrainType(cll, TERRAIN_ROCK))
					iC++;
				if (isAboveSpecificTerrainType(cll, TERRAIN_ROCK))
					iC++;
				if (isBelowSpecificTerrainType(cll, TERRAIN_ROCK))
					iC++;

				if (isLeftSpecificTerrainType(cll, TERRAIN_MOUNTAIN))
					iC++;
				if (isRightSpecificTerrainType(cll, TERRAIN_MOUNTAIN))
					iC++;
				if (isAboveSpecificTerrainType(cll, TERRAIN_MOUNTAIN))
					iC++;
				if (isBelowSpecificTerrainType(cll, TERRAIN_MOUNTAIN))
					iC++;
