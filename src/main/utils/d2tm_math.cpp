...
int convert_angle(int face_angle) {
	...
	if (face_angle == FACE_RIGHT)
		return 0;
	if (face_angle == FACE_UPRIGHT)
		return 1;
	if (face_angle == FACE_UP)
		return 2;
	if (face_angle == FACE_UPLEFT)
		return 3;
	if (face_angle == FACE_LEFT)
		return 4;
	if (face_angle == FACE_DOWNLEFT)
		return 5;
	if (face_angle == FACE_DOWN)
		return 6;
	if (face_angle == FACE_DOWNRIGHT)
		return 7;
...
bool CELL_BORDERS(int iOrigin, int iCell) {
	...
	if (CELL_ABOVE(iOrigin) == iCell)
		return true;
	if (CELL_UNDER(iOrigin) == iCell)
		return true;
	if (CELL_LEFT(iOrigin) == iCell)
		return true;
	if (CELL_RIGHT(iOrigin) == iCell)
		return true;

	//
	if (CELL_U_LEFT(iOrigin) == iCell)
		return true;
	if (CELL_U_RIGHT(iOrigin) == iCell)
		return true;
	if (CELL_L_LEFT(iOrigin) == iCell)
		return true;
	if (CELL_L_RIGHT(iOrigin) == iCell)
		return true;
