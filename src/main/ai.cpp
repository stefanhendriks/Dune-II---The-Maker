...
void cAIPlayer::BUILD_UNIT(int iUnitType) {
	...
	if (player[ID].getHouse() == HARKONNEN || player[ID].getHouse() == SARDAUKAR) {
		if (iUnitType == INFANTRY)
			iUnitType = TROOPERS;
		if (iUnitType == SOLDIER)
			iUnitType = TROOPER;
		if (iUnitType == TRIKE)
			iUnitType = QUAD;
	}
	...
	for (int i = 0; i < MAX_UNITTYPES; i++) {
		...
		if (iUnitType == TANK || iUnitType == LAUNCHER || iUnitType == SIEGETANK || iUnitType == SONICTANK || iUnitType == DEVASTATOR || iUnitType == HARVESTER) {
			if (i == iUnitType)
				if (iBuildingUnit[i] > -1)
					bAlreadyBuilding = true;
		}
...
void cAIPlayer::think_harvester() {
	...
	for (int c = 0; c < MAX_CELLS; c++)
		if (map->cell[c].terrainTypeGfxDataIndex == TERRAIN_BLOOM)
			iBlooms++;
	...
	if (iBlooms < 3) {
		...
		if (rnd(100) < 15) {
			...
			for (int i = 0; i < MAX_UNITS; i++) {
				if (unit[i].isValid()) {
					if (unit[i].iPlayer > 0 && unit[i].iPlayer == ID && unit[i].iAction == ACTION_GUARD) {
...
void cAIPlayer::think_attack() {
	...
	if (player[ID].iStructures[WOR] > 0 || player[ID].iStructures[BARRACKS])
		if (rnd(100) < 30) {
			bInfantryOnly = true;
			iAmount = 10;
		}
	...
	for (int i = 0; i < MAX_UNITS; i++) {
		if (unit[i].isValid())
			if (unit[i].iPlayer == 0) {
				if (unit[i].iType != HARVESTER && unit[i].iType != CARRYALL)
					iThem++;
			} else {
				if (unit[i].iType != HARVESTER && unit[i].iType != CARRYALL)
					iWe++;
	...
	for (int i = 0; i < MAX_UNITS; i++) {
		if (unit[i].isValid())
			if (unit[i].iPlayer == ID && unit[i].iType != HARVESTER && unit[i].iType != CARRYALL) {
	...
	if (iUnits < iAmount && rnd(100) < 10 && player[ID].credits > 700) {
		for (int i = 0; i < MAX_UNITS; i++) {
			if (unit[i].isValid())
				if (unit[i].iPlayer == ID) {
					if (unit[i].iType != HARVESTER && unit[i].iType != CARRYALL && unit[i].iType != FRIGATE && unit[i].iAction == ACTION_GUARD) {
						if (bUnit)
							UNIT_ORDER_ATTACK(i, unit[iTarget].iCell, iTarget, -1, -1);
						else
							UNIT_ORDER_ATTACK(i, structure[iTarget]->getCell(), -1, iTarget, -1);
			...
			if (unit[i].iType == SANDWORM && unit[i].iPlayer == ID) {
				...
				if (unit[i].iAction == ACTION_GUARD) {
					...
					for (int iTries = 0; iTries < 10; iTries++) {
						...
						if (map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_SAND || map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_HILL
								|| map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_SPICE || map->cell[iMoveTo].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL) {
	...
	for (int i = 0; i < MAX_UNITS; i++)
		if (unit[i].isValid())
			if (unit[i].iPlayer == iAttackPlayer) {
				if (DEBUGGING) {
					char msg[255];
					sprintf(msg, "AI %d (House %d) -> Visible = %d", iPlayer, player[iPlayer].getHouse(),
							mapUtils->isCellVisibleForPlayerId(iPlayer, unit[i].iCell));
					logbook(msg);
				}

				if (mapUtils->isCellVisibleForPlayerId(iPlayer, unit[i].iCell) || game.bSkirmish) {
					iTargets[iT] = i;
					iT++;
	...
	for (int i = 0; i < MAX_STRUCTURES; i++)
		if (structure[i])
			if (structure[i]->getOwner() == iAttackPlayer)
				if (mapUtils->isCellVisibleForPlayerId(iPlayer, structure[i]->getCell()) || game.bSkirmish) {
					iTargets[iT] = i;
