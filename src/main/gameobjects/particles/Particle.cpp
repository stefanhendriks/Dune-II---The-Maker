...
void Particle::think() {/*
 ...
 return;
	...
	if (iType == OBJECT_SMOKE || iType == OBJECT_SMOKE_SHADOW) {
		TIMER_frame--;
		TIMER_dead--;
	...
	if (iType == OBJECT_DEADINF01 || iType == OBJECT_DEADINF02) {
		TIMER_frame--;
		TIMER_dead--;
	...
	// tank explosion(s)
	if (iType == EXPLOSION_TANK_ONE || iType == EXPLOSION_TANK_TWO || iType == EXPLOSION_STRUCTURE01 || iType == EXPLOSION_STRUCTURE02 || iType == EXPLOSION_GAS
	...
	if (iType == EXPLOSION_SQUISH01 || iType == EXPLOSION_SQUISH02 || iType == EXPLOSION_SQUISH03 || iType == EXPLOSION_ORNI) {
		TIMER_frame--;
		if (TIMER_frame < 0) {
			if (iAlpha > 5)
				iAlpha -= 5;
