...
void cUnit::die(bool bBlowUp, bool bSquish) {
	...
	if (iType == HARVESTER && player[iPlayer].iStructures[REFINERY] > 0) {
		if (DEBUGGING)
	...
	if (bBlowUp) {
		...
		if (iType == TANK || iType == SIEGETANK || iType == SONICTANK || iType == LAUNCHER || iType == DEVIATOR || iType == HARVESTER || iType == ORNITHOPTER
				|| iType == MCV) {
			// play quick 'boom' sound and show animation
			if (rnd(100) < 50) {
				...
				for (int cy = 0; cy < 3; cy++) {
					...
					if (map->cell[cll].gameObjectId[MAPID_STRUCTURES] > -1) {
						...
						if (structure[id]->getHitPoints() > 0) {
							...
							if (rnd(100) < iChance)
								PARTICLE_CREATE(draw_x() + (mapCamera->getX() * 32) + 16 + (-8 + rnd(16)), draw_y() + (mapCamera->getY() * 32) + 16
										+ (-8 + rnd(16)), OBJECT_SMOKE, -1, -1);
					...
					} else if (map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SAND || map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_HILL
							|| map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SPICE || map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL) {
						if (map->cell[cll].terrainTypeGfxDataIndex != TERRAIN_WALL)
							map->cell[cll].hitpoints -= 30;
...
bool cUnit::isValid() {
	...
	//    char msg[255];
	//    sprintf(msg, "iPlayerId is [%d], hitpoints is [%d], iTempHitpoints is [%d], iCell is [%d]", iPlayer, iHitPoints, iTempHitPoints, iCell);
	//    logbook(msg);
...
float cUnit::fExpDamage() {
	if (fExperience < 1)
		return 0; // no stars, no increasement
			...
			if (iType == SANDWORM) {
				...
				for (int i = 0; i < MAX_UNITS; i++) {
					if (unit[i].isValid() && i != iID) {
						...
						if (unit[i].iPlayer != iPlayer && units[unit[i].iType].airborn == false) {
							if (map->cell[unit[i].iCell].terrainTypeGfxDataIndex == TERRAIN_SAND || map->cell[unit[i].iCell].terrainTypeGfxDataIndex
									== TERRAIN_HILL || map->cell[unit[i].iCell].terrainTypeGfxDataIndex == TERRAIN_SPICE
			...
			} else // not sandworm
			{
				...
				for (int i = 0; i < MAX_UNITS; i++) {
					if (unit[i].isValid() && i != iID) {
						...
						// not ours and its visible
						if (unit[i].iPlayer != iPlayer && mapUtils->isCellVisibleForPlayerId(iPlayer, unit[i].iCell) && // is visible for ai as well?
			...
			if (iType != SANDWORM && iPlayer > 0) {
				...
				for (int i = 0; i < MAX_STRUCTURES; i++) {
					if (structure[i] && i != iID) {
						...
						// not ours and its visible
						if (structure[i]->getOwner() != iPlayer && mapUtils->isCellVisibleForPlayerId(iPlayer, structure[i]->getCell()) && bAlly == false) {
							int distance = ABS_length(iCellX, iCellY, iCellGiveX(structure[i]->getCell()), iCellGiveY(structure[i]->getCell()));
...
void cUnit::think() {
	...
	if (map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_BLOOM && units[iType].airborn == false) {
		...
		if (iAttackUnit < 0 && TIMER_attack == 0) {
			...
			for (int i = 0; i < MAX_UNITS; i++) {
				if (unit[i].isValid() && i != iID) {
					...
					// not ours and its visible
					if (unit[i].iPlayer != iPlayer && mapUtils->isCellVisibleForPlayerId(iPlayer, unit[i].iCell) && units[unit[i].iType].airborn == false && bAlly
							== false) {
						int distance = ABS_length(iCellX, iCellY, unit[i].iCellX, unit[i].iCellY);
	...
	if (iType == HARVESTER) {
		...
		if (iCell == iGoalCell) {
			...
			if (map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_SPICE || map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL) {
				// do timer stuff
			...
			if (TIMER_harvest > (difficultySettings->getHarvestSpeed(units[iType].harvesting_speed)) && iCredits < units[iType].credit_capacity) {
				TIMER_harvest = 1;
		...
		if (iStructureID > -1 && map->cell[iCell].gameObjectId[MAPID_STRUCTURES] == iStructureID && map->cell[iCell].gameObjectId[MAPID_STRUCTURES] > -1) {
	...
	if (iType == CARRYALL) {
		...
		if ((iTransferType == TRANSFER_NEW_STAY || iTransferType == TRANSFER_NEW_LEAVE || iTransferType == TRANSFER_PICKUP) || iUnitID > -1) {
...
void cUnit::think_move_air() {
	...
	if (iNextCell == iCell) {
				...
				if (bPickedUp == false) {
					if (unit[iUnitID].isValid() && unit[iUnitID].bPickedUp == false) {
						...
						if (iCell == iTheGoal) {
							...
							if (unit[iUnitID].iOffsetX == 0 && unit[iUnitID].iOffsetY == 0) {
								// pick this up baby
		...
		if (iTransferType == TRANSFER_NEW_LEAVE || iTransferType == TRANSFER_NEW_STAY) {
			// bring a new unit
	...
	if (iUnitID > -1 || (iTransferType != TRANSFER_DIE && iTransferType != TRANSFER_NONE)) {
		...
		if (iType != FRIGATE) {
			if (iLength > 8)
				...
				if (rnd(100) < 5) {
					if (map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_SAND || map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_SPICE
							|| map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_HILL || map->cell[iCell].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL) {
						int iDieX = (draw_x() + 16) + (mapCamera->getX() * 32);
						int iDieY = (draw_y() + 16) + (mapCamera->getY() * 32);
						PARTICLE_CREATE(iDieX, iDieY, OBJECT_CARRYPUFF, -1, -1);
					}
	...
	if (iType == SANDWORM) {
		if (iAttackUnit > -1) {
			...
			if (iGoalCell == iCell) {
				...
				if (map->cell[unit[iAttackUnit].iCell].terrainTypeGfxDataIndex != TERRAIN_SAND && map->cell[unit[iAttackUnit].iCell].terrainTypeGfxDataIndex
						!= TERRAIN_HILL && map->cell[unit[iAttackUnit].iCell].terrainTypeGfxDataIndex != TERRAIN_SPICE
...
void cUnit::think_move() {
	...
	if (map->cell[iNextCell].gameObjectId[MAPID_UNITS] > -1 && map->cell[iNextCell].gameObjectId[MAPID_UNITS] != iID) {
		// get it
	...
	if (iStructureID > -1 && map->cell[iNextCell].gameObjectId[MAPID_STRUCTURES] != iStructureID && map->cell[iNextCell].gameObjectId[MAPID_STRUCTURES] > -1)
		bOccupied = true;
	...
	if (iBodyShouldFace == iBodyFacing) {
		...
		if (iType != SANDWORM)
			...
			if ((iOffX == 8 || iOffX == 16 || iOffX == 24 || iOffX == 32) || (iOffY == 8 || iOffY == 16 || iOffY == 24 || iOffY == 32)) {
				int iParX = (draw_x() + 16) + (mapCamera->getX() * 32);
				int iParY = (draw_y() + 16) + (mapCamera->getY() * 32);
		...
		if (iOffsetX == 0 && iOffsetY == 0) {
			...
			if (map->cell[iCell].terrainTypeGfxDataIndex != TERRAIN_ROCK && map->cell[iCell].terrainTypeGfxDataIndex != TERRAIN_MOUNTAIN
					&& map->cell[iCell].terrainTypeGfxDataIndex != TERRAIN_WALL && map->cell[iCell].terrainTypeGfxDataIndex != TERRAIN_SLAB
...
int UNIT_CREATE(int iCll, int iTpe, int iPlyr, bool bOnStart) {
	...
	if (iTpe == SANDWORM) {
		if (map->cell[iCll].terrainTypeGfxDataIndex != TERRAIN_SAND && map->cell[iCll].terrainTypeGfxDataIndex != TERRAIN_SPICE
				&& map->cell[iCll].terrainTypeGfxDataIndex != TERRAIN_HILL && map->cell[iCll].terrainTypeGfxDataIndex != TERRAIN_SPICEHILL) {
			return -1;
		}
	}
...
// 	}
	...
	if (map->occupied(CELL_LEFT(iCell), iID) && map->occupied(CELL_RIGHT(iCell), iID) && map->occupied(CELL_ABOVE(iCell), iID) && map->occupied(CELL_UNDER(iCell),
			iID) && map->occupied(CELL_L_LEFT(iCell), iID) && map->occupied(CELL_L_RIGHT(iCell), iID) && map->occupied(CELL_U_RIGHT(iCell), iID) && map->occupied(
			CELL_U_LEFT(iCell), iID)) {
		...
		for (cx = sx; cx <= ex; cx++) {
			...
			for (cy = sy; cy <= ey; cy++) {
				...
				if (is_worm == false) {
					...
					if (map->cell[cll].gameObjectId[MAPID_UNITS] > -1) {
						...
						if (map->cell[cll].gameObjectId[MAPID_UNITS] != iID) {
							...
							if (iPathCountUnits != 0)
								if (iPathCountUnits <= 0) {
									if (map->cell[cll].gameObjectId[MAPID_UNITS] != -1 && map->cell[cll].gameObjectId[MAPID_UNITS] != iID) // occupied by a unit
										good = false;
								}

							if (unit[iUID].iPlayer != unit[iID].iPlayer)
								if (units[unit[iUID].iType].infantry && units[unit[iID].iType].squish) // and the current unit can squish
					...
					if (map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SAND || map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SPICE
							|| map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_HILL || map->cell[cll].terrainTypeGfxDataIndex == TERRAIN_SPICEHILL) {
						good = true;
					} else
						good = false;
					//good=true;
	...
	while (bSearch) {
		...
		for (int iSX = iStartX; iSX < iEndX; iSX++)
			for (int iSY = iStartY; iSY < iEndY; iSY++) {
				...
				if ((map->cell[cll].gameObjectId[MAPID_STRUCTURES] < 0) && (map->cell[cll].gameObjectId[MAPID_UNITS] < 0)) {
...
int CARRYALL_TRANSFER(int iuID, int iGoal) {
	...
	for (int iCarry = 0; iCarry < MAX_UNITS; iCarry++) {
		if (unit[iCarry].isValid() && unit[iCarry].iPlayer == unit[iuID].iPlayer && unit[iCarry].iType == CARRYALL) {
