...
void cStarPort::think_deploy()
{
    if (frigateDroppedPackage) {
        ...
        if (TIMER_deploy < 0) {
            TIMER_deploy = 1;
            // deploy unit
            cOrderProcesser *orderProcesser = g_Player[iPlayer].getOrderProcesser();
            ...
            if (item) {
                ...
                if (cellToDeployTo >= 0) {
                    ...
                    if (rallyPoint > -1) {
                        g_Unit[id].move_to(rallyPoint, -1, -1);
                    }
                    ...
                    int cellAtBorderOfMap = global_map.findCloseMapBorderCellRelativelyToDestinationCel(cellToDeployTo);
