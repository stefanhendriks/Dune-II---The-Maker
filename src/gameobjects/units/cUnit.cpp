...
void cUnit::die(bool bBlowUp, bool bSquish)
{
    ...
    if (iStructureID > -1) {
        cAbstractStructure *pStructure = game.getStructures()[iStructureID];
        ...
        for (int cx = 0; cx < 3; cx++)
            for (int cy = 0; cy < 3; cy++) {
                ...
                if (idOfStructureAtCell > -1) {
                    ...
                    cAbstractStructure *pStructure = game.getStructures()[id];
                    assert(pStructure);
...
void cUnit::attackStructure(int targetStructure)
{
    ...
    attack(game.getStructures()[targetStructure]->getCell(), -1, targetStructure, -1, true);
}
...
void cUnit::move_to(int iCll, int iStructureIdToEnter, int iUnitIdToPickup, eUnitActionIntent intent)
{
    ...
    if (iStructureIdToEnter > -1) {
        cAbstractStructure *pStructure = game.getStructures()[iStructureIdToEnter];
        ...
        if (m_transferType == eTransferType::NEW_LEAVE ||
            ...
            if (iType == FRIGATE) {
                ...
                if (iStrucId > -1) {
                    ...
                    game.getStructures()[iStrucId]->setFrame(4); // show package on this structure
                    game.getStructures()[iStrucId]->setAnimating(true); // keep animating
                    dynamic_cast<cStarPort *>(game.getStructures()[iStrucId])->setFrigateDroppedPackage(true);
                }
                else {
...
cAbstractStructure *cUnit::getStructureUnitWantsToEnter() const
{
    ...
    if (iStructureID > -1) {
        structureUnitWantsToEnter = game.getStructures()[iStructureID];
                ...
                if (notAttacking) {
                    ...
                    if (iDestCell < 0) {
                        ...
                        if (iAttackStructure > -1) {
                            cAbstractStructure *pStructure = game.getStructures()[iAttackStructure];
...
void cUnit::think_attack()
{
    ...
    if (iAttackStructure > -1) {
        pStructure = game.getStructures()[iAttackStructure];
        if (pStructure && pStructure->isValid()) {
...
eUnitMoveToCellResult cUnit::moveToNextCellLogic()
{
    ...
    if (!isMovingBetweenCells()) {
        ...
        if (iPlayer == AI_CPU5 && game.getPlayer(HUMAN).isHouse(ATREIDES)) {
            ...
            game.getMap().clearShroud(iCell, sUnitInfo[iType].sight, HUMAN);
        }

        game.getMap().clearShroud(iCell, sUnitInfo[iType].sight, iPlayer);
    ...
    if (isDead()) {
        ...
        if (iHitPoints < getUnitInfo().dieWhenLowerThanHP) {
            ...
            if (unitWhoDealsDamage > -1) {
                ...
                cAbstractStructure *pStructure = game.getStructures()[structureWhoDealsDamage];
...
void cUnit::think_harvester()
{
    ...
    if (iCell == iGoalCell) {
        ...
        if (TIMER_harvest > (difficultySettings->getHarvestSpeed(sUnitInfo[iType].harvesting_speed)) &&
            ...
            game.getMap().cellTakeCredits(iCell, sUnitInfo[iType].harvesting_amount);
...
void cUnit::retreatToNearbyBase()
{
    ...
    cAbstractStructure *pStructure = game.getStructures()[closest.entityId];
