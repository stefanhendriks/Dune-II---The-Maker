...
void cUnit::draw()
{
    ...
    cPlayer &cPlayer = g_Players[this->iPlayer];
...
void cUnit::log(const std::string &txt) const
{
    ...
    g_Players[iPlayer].log(std::format("[UNIT[{}]: type = {}(={}), iCell = {}, iGoalCell = {}] '{}'",
...
eUnitMoveToCellResult cUnit::moveToNextCellLogic()
{
    ...
    cPlayerDifficultySettings *difficultySettings = g_Players[iPlayer].getDifficultySettings();
    if (TIMER_move < ((difficultySettings->getMoveSpeed(iType, iSlowDown)))) {
    ...
    if (!isMovingBetweenCells()) {
        ...
        if (iPlayer == AI_CPU5 && g_Players[HUMAN].isHouse(ATREIDES)) {
