...
cInfoContextCreator::cInfoContextCreator(GraphicsContext* graphicsContext) {
    d2tm_assert(graphicsContext != nullptr);
    m_gfxdata = graphicsContext->gfxdata.get();
...
void cInfoContextCreator::initUnits(cUnitInfos& unitInfos)
{
    ...
    for (int i = 0; i < MAX_UNITTYPES; i++) {
        ...
        unitInfo.bmp = m_gfxdata->getSurface(UNIT_QUAD); // default bitmap is a quad!
    ...
    unitInfos[CARRYALL].bmp = m_gfxdata->getSurface(UNIT_CARRYALL);      // pointer to the original 8bit bitmap
    unitInfos[CARRYALL].shadow = m_gfxdata->getTexture(UNIT_CARRYALL_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[ORNITHOPTER].bmp = m_gfxdata->getSurface(UNIT_ORNITHOPTER);      // pointer to the original 8bit bitmap
    unitInfos[ORNITHOPTER].shadow = m_gfxdata->getTexture(UNIT_ORNITHOPTER_SHADOW);      // pointer to the original 8bit bitmap
...
//    unitInfos[ORNITHOPTER].canAttackAirUnits = true; // orni's can attack other air units
    ...
    unitInfos[DEVASTATOR].bmp = m_gfxdata->getSurface(UNIT_DEVASTATOR);      // pointer to the original 8bit bitmap
    unitInfos[DEVASTATOR].shadow = m_gfxdata->getTexture(UNIT_DEVASTATOR_SHADOW);      // pointer to the original bitmap shadow
    ...
    unitInfos[HARVESTER].bmp = m_gfxdata->getSurface(UNIT_HARVESTER);      // pointer to the original 8bit bitmap
    unitInfos[HARVESTER].shadow = m_gfxdata->getTexture(UNIT_HARVESTER_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[TANK].bmp = m_gfxdata->getSurface(UNIT_TANKBASE);      // pointer to the original 8bit bitmap
    unitInfos[TANK].shadow = m_gfxdata->getTexture(UNIT_TANKBASE_SHADOW);      // pointer to the original 8bit bitmap
    unitInfos[TANK].top = m_gfxdata->getSurface(UNIT_TANKTOP);      // pointer to the original 8bit bitmap
    ...
    unitInfos[SIEGETANK].bmp = m_gfxdata->getSurface(UNIT_SIEGEBASE);      // pointer to the original 8bit bitmap
    unitInfos[SIEGETANK].shadow = m_gfxdata->getTexture(UNIT_SIEGEBASE_SHADOW);      // pointer to the original 8bit bitmap
    unitInfos[SIEGETANK].top = m_gfxdata->getSurface(UNIT_SIEGETOP);      // pointer to the original 8bit bitmap
    ...
    unitInfos[MCV].bmp = m_gfxdata->getSurface(UNIT_MCV);      // pointer to the original 8bit bitmap
    unitInfos[MCV].shadow = m_gfxdata->getTexture(UNIT_MCV_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[DEVIATOR].bmp = m_gfxdata->getSurface(UNIT_DEVIATOR);      // pointer to the original 8bit bitmap
    ...
    unitInfos[LAUNCHER].bmp = m_gfxdata->getSurface(UNIT_LAUNCHER);      // pointer to the original 8bit bitmap
    unitInfos[LAUNCHER].shadow = m_gfxdata->getTexture(UNIT_LAUNCHER_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[QUAD].bmp = m_gfxdata->getSurface(UNIT_QUAD);      // pointer to the original 8bit bitmap
    unitInfos[QUAD].shadow = m_gfxdata->getTexture(UNIT_QUAD_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[TRIKE].bmp = m_gfxdata->getSurface(UNIT_TRIKE);      // pointer to the original 8bit bitmap
    unitInfos[TRIKE].shadow = m_gfxdata->getTexture(UNIT_TRIKE_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[RAIDER].bmp = m_gfxdata->getSurface(UNIT_TRIKE);      // pointer to the original 8bit bitmap
    unitInfos[RAIDER].shadow = m_gfxdata->getTexture(UNIT_TRIKE_SHADOW);      // pointer to the original 8bit bitmap
    ...
    // Unit        : Frigate
    // Description : Frigate
    unitInfos[FRIGATE].bmp = m_gfxdata->getSurface(UNIT_FRIGATE);      // pointer to the original 8bit bitmap
    unitInfos[FRIGATE].shadow = m_gfxdata->getTexture(UNIT_FRIGATE_SHADOW);      // pointer to the original 8bit bitmap
    ...
    */
    ...
    unitInfos[SONICTANK].bmp = m_gfxdata->getSurface(UNIT_SONICTANK);      // pointer to the original 8bit bitmap
    unitInfos[SONICTANK].shadow = m_gfxdata->getTexture(UNIT_SONICTANK_SHADOW);      // pointer to the original 8bit bitmap
    ...
    unitInfos[SOLDIER].bmp = m_gfxdata->getSurface(UNIT_SOLDIER);      // pointer to the original 8bit bitmap
    ...
    unitInfos[INFANTRY].bmp = m_gfxdata->getSurface(UNIT_SOLDIERS);      // pointer to the original 8bit bitmap
    ...
    unitInfos[TROOPER].bmp = m_gfxdata->getSurface(UNIT_TROOPER);      // pointer to the original 8bit bitmap
    ...
    unitInfos[TROOPERS].bmp = m_gfxdata->getSurface(UNIT_TROOPERS);      // pointer to the original 8bit bitmap
    ...
    unitInfos[UNIT_FREMEN_ONE].bmp = m_gfxdata->getSurface(UNIT_SINGLEFREMEN);      // pointer to the original 8bit bitmap
...
//  units[UNIT_FREMEN_ONE].subListId=0;
    ...
    unitInfos[UNIT_FREMEN_THREE].bmp = m_gfxdata->getSurface(UNIT_TRIPLEFREMEN);      // pointer to the original 8bit bitmap
...
//  units[UNIT_FREMEN_THREE].subListId=0;
    ...
    unitInfos[SABOTEUR].bmp = m_gfxdata->getSurface(UNIT_SABOTEUR);
    ...
    unitInfos[SANDWORM].bmp = m_gfxdata->getSurface(UNIT_SANDWORM);
...
void cInfoContextCreator::initBullets(cBulletInfos& bulletInfos)
{
    ...
    // huge rocket/missile
    bulletInfos[ROCKET_BIG].bmp = m_gfxdata->getTexture(BULLET_ROCKET_LARGE);
    ...
    bulletInfos[ROCKET_SMALL_ORNI].bmp = m_gfxdata->getTexture(BULLET_ROCKET_SMALL);
    ...
    // small rocket
    bulletInfos[ROCKET_SMALL].bmp = m_gfxdata->getTexture(BULLET_ROCKET_SMALL);
...
//    bullets[ROCKET_SMALL].smokeParticle = BULLET_PUF; // small rockets have no smoke trail yet
    ...
    bulletInfos[ROCKET_SMALL_FREMEN].bmp = m_gfxdata->getTexture(BULLET_ROCKET_SMALL);
...
//    bullets[ROCKET_SMALL_FREMEN].smokeParticle = true; // not yet
    ...
    // normal rocket
    bulletInfos[ROCKET_NORMAL].bmp = m_gfxdata->getTexture(BULLET_ROCKET_NORMAL);
    ...
    // soldier shot
    bulletInfos[BULLET_SMALL].bmp = m_gfxdata->getTexture(BULLET_DOT_SMALL);
    ...
    // trike shot
    bulletInfos[BULLET_TRIKE].bmp = m_gfxdata->getTexture(BULLET_DOT_SMALL);
    ...
    // quad shot
    bulletInfos[BULLET_QUAD].bmp = m_gfxdata->getTexture(BULLET_DOT_SMALL);
    ...
    // normal tank shot
    bulletInfos[BULLET_TANK].bmp = m_gfxdata->getTexture(BULLET_DOT_MEDIUM);
    ...
    // siege tank shot
    bulletInfos[BULLET_SIEGE].bmp = m_gfxdata->getTexture(BULLET_DOT_MEDIUM);
    ...
    // devastator shot
    bulletInfos[BULLET_DEVASTATOR].bmp = m_gfxdata->getTexture(BULLET_DOT_LARGE);
    ...
    bulletInfos[BULLET_GAS].bmp = m_gfxdata->getTexture(BULLET_ROCKET_NORMAL);
    ...
    // normal turret shot
    bulletInfos[BULLET_TURRET].bmp = m_gfxdata->getTexture(BULLET_DOT_MEDIUM);
    ...
    bulletInfos[ROCKET_RTURRET].bmp = m_gfxdata->getTexture(BULLET_ROCKET_NORMAL);
...
void cInfoContextCreator::initStructures(cStructureInfos& structureInfos)
{
    ...
    for (int i = 0; i < MAX_STRUCTURETYPES; i++) {
        ...
        structureInfo.bmp = m_gfxdata->getSurface(BUILD_WINDTRAP); // in case an invalid bitmap, we are a windtrap
    ...
    // Single and 4 slabs
    structureInfos[SLAB1].bmp = m_gfxdata->getSurface(PLACE_SLAB1); // in case an invalid bitmap, we are a windtrap
    ...
    structureInfos[SLAB4].bmp = m_gfxdata->getSurface(PLACE_SLAB4); // in case an invalid bitmap, we are a windtrap
    ...
    // Concrete Wall
    structureInfos[WALL].bmp = m_gfxdata->getSurface(PLACE_WALL); // in case an invalid bitmap, we are a windtrap
    ...
    // Structure    : Windtrap
    // Description  : <none>
    structureInfos[WINDTRAP].bmp = m_gfxdata->getSurface(BUILD_WINDTRAP);
    structureInfos[WINDTRAP].shadow = m_gfxdata->getTexture(BUILD_WINDTRAP_SHADOW); // shadow
    ...
    structureInfos[HEAVYFACTORY].bmp = m_gfxdata->getSurface(BUILD_HEAVYFACTORY);
    structureInfos[HEAVYFACTORY].shadow = m_gfxdata->getTexture(BUILD_HEAVYFACTORY_SHADOW); // shadow
    structureInfos[HEAVYFACTORY].flash = m_gfxdata->getSurface(BUILD_HEAVYFACTORY_FLASH);
    ...
    structureInfos[HIGHTECH].bmp = m_gfxdata->getSurface(BUILD_HIGHTECH);
    structureInfos[HIGHTECH].shadow = m_gfxdata->getTexture(BUILD_HIGHTECH_SHADOW);
    ...
    structureInfos[REPAIR].bmp = m_gfxdata->getSurface(BUILD_REPAIR);
    structureInfos[REPAIR].shadow = m_gfxdata->getTexture(BUILD_REPAIR_SHADOW);
    ...
    structureInfos[PALACE].bmp = m_gfxdata->getSurface(BUILD_PALACE);
    structureInfos[PALACE].shadow = m_gfxdata->getTexture(BUILD_PALACE_SHADOW);
    ...
    structureInfos[LIGHTFACTORY].bmp = m_gfxdata->getSurface(BUILD_LIGHTFACTORY);
    structureInfos[LIGHTFACTORY].shadow = m_gfxdata->getTexture(BUILD_LIGHTFACTORY_SHADOW);
    structureInfos[LIGHTFACTORY].flash = m_gfxdata->getSurface(BUILD_LIGHTFACTORY_FLASH);
    ...
    structureInfos[RADAR].bmp = m_gfxdata->getSurface(BUILD_RADAR);
    structureInfos[RADAR].shadow = m_gfxdata->getTexture(BUILD_RADAR_SHADOW); // shadow
    ...
    structureInfos[BARRACKS].bmp = m_gfxdata->getSurface(BUILD_BARRACKS);
    structureInfos[BARRACKS].shadow = m_gfxdata->getTexture(BUILD_BARRACKS_SHADOW);
    ...
    structureInfos[WOR].bmp = m_gfxdata->getSurface(BUILD_WOR);
    structureInfos[WOR].shadow = m_gfxdata->getTexture(BUILD_WOR_SHADOW);
    ...
    structureInfos[SILO].bmp = m_gfxdata->getSurface(BUILD_SILO);
    structureInfos[SILO].shadow = m_gfxdata->getTexture(BUILD_SILO_SHADOW);
    ...
    structureInfos[REFINERY].bmp = m_gfxdata->getSurface(BUILD_REFINERY);
    structureInfos[REFINERY].shadow = m_gfxdata->getTexture(BUILD_REFINERY_SHADOW);
    ...
    structureInfos[CONSTYARD].bmp = m_gfxdata->getSurface(BUILD_CONSTYARD);
    structureInfos[CONSTYARD].shadow = m_gfxdata->getTexture(BUILD_CONSTYARD_SHADOW);
    ...
    structureInfos[STARPORT].bmp = m_gfxdata->getSurface(BUILD_STARPORT);
    structureInfos[STARPORT].shadow = m_gfxdata->getTexture(BUILD_STARPORT_SHADOW);
    ...
    structureInfos[IX].bmp = m_gfxdata->getSurface(BUILD_IX);
    structureInfos[IX].shadow = m_gfxdata->getTexture(BUILD_IX_SHADOW);
    ...
    structureInfos[TURRET].bmp = m_gfxdata->getSurface(BUILD_TURRET);
    structureInfos[TURRET].shadow = m_gfxdata->getTexture(BUILD_TURRET_SHADOW);
    ...
    structureInfos[RTURRET].bmp = m_gfxdata->getSurface(BUILD_RTURRET);
    structureInfos[RTURRET].shadow = m_gfxdata->getTexture(BUILD_RTURRET_SHADOW);
